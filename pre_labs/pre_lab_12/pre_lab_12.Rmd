...
**Answer** It outputs a database for many of the Ben Cardin press releases with 99 rows and four columns: data, title, url and text.
...
**Answer** The "unique" function takes one of each word (or punctuation) with different spelling or capitalization and removes any duplicates in the list.
...
**Answer** The "unnest_tokens" functions took each of the words from the press releases and put them in a single column data table. As you go down the column, the words can be mentally strung together to form a sentence. 
...
**Answer** The result is not really interesting, as many of the top words in the graphic are relatively common words in the English language.
...
**Answer** The result is much more interesting, as many of the top words are more relevant to American policy making and local communities. 
...
**Answer** "COVID" and "health" were among the top 10 words in 2021, which makes sense considering that the pandemic was still lingering at the time. Similarly, "health" was one of the top 10 words in 2022, and the word "project" taking the top spot that year makes me believe that the state was passing legislation to improve the welfare/health care of Americans. For 2023, the top 10 words are of many varying topics, like "health," "women" and "fire."
...
**Task** Run the following code and describe the results.
**Answer** "COVID-19" took the top spot for 2021, but "19 pandemic" was #3, so it makes me wonder if using a trigram is better for creating the database. The code also recognized names of various places like "Chesapeake Bay" and "Baltimore City," as well as the name "John Sarbanes."
...
**Task** Run the following code and describe the results.
**Answer** "COVID-19" now takes the #4 spot for the list, so it seems it was less of a priority for Maryland policy makers that year. "Health care" did move up to the #3 position, and there are terms like "human rights" making their first appearance on the list.
...
**Task** Run the following code and describe the results.
**Answer** "COVID-19" is no longer on the top 10 list, while "human rights" now takes the top spot. Similarly, "equal rights" comes close at the #2 spot. There are some more surprising terms making their first appearances on this list, particularly "oral health" and "offshore wind."
...
**Answer** Many of the sentiments seem to correspond with many of the words on the list (e.g., positive for "relief," negative for "emergency"), but some of the other words (e.g., "isolated," "limit," "fallen") appear too generalized and would need more context in order to properly determine whether they present a positive or negative (and maybe even neutral) sentiment.
