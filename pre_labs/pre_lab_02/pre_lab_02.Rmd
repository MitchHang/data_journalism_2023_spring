...
**Answer** This variable is to differentiate from the previous journalism_courses variable we created a while back so we don't overwrite it.
...
```{r}
umd_courses %>% 
  filter(str_detect(instructors, "Keith Yearwood"))
```
...
**Answer:** The expenditure_date column has changed its formatting from MM/DD/YY to YYYY-MM-DD, and it has changed from <chr> to <date>.
...
**Answer** I think the total amounts are declared for the first of the month, and the values are not necessarily sorted chronologically (newest or oldest month first), but rather by their corresponding total amount. I'm guessing the "-01" at the end is a placeholder representing the entire month.
...
**Answer** I think the code is calculating what percentage of the votes Wes Moore got out of all votes for the candidates (and write-ins) combined.
...
**Answer** It sorts the counties in which Wes Moore got the highest percentage of total votes in descending order.
...
**Answer** I would say maybe not, because you're comparing the performance of Wes Moore in smaller counties (such as Somerset) compared to larger counties (such as Prince George's).
...
**Answer** str_to_lower() converts to lower case ("bank of america"), and str_to_title() converts to title case ("Bank of America").
...
**Answer** I think the mutate statement is analyzing the name of a state in the address field, then creating a new column and adding the abbreviation for the state. Otherwise, it would put N/A in the field for everything else.
...
**Answer** I think this answer does make sense because it is simply gathering all of the total payments made for other states and putting them in a simple two-column tibble. 
...
1. What was the total payment amount for the District of Columbia (DC)?

2. What was the percentage of total votes that Dan Cox attained in each county? How many counties did he recieve the majority vote?

3. What was the total amount of payments made towards Bank of America for the month of June 2022?
