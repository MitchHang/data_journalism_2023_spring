...
**Answer** It outputs the name of the states (plus D.C.), the declared 2016 presidential candidate winner/front-runner in the state, the percentage of people in the state who voted for Trump, the percentage of businesses receiving PPE in each state, and the PPE amount per employee in each state.
```{r}
...
**Answer** I see a bunch of points being conglomerated in the top right corner of the plot, indicating that there is a positive relationship between the percentage of votes for Trump and the percentage of businesses receiving PPE. The trend line seems to be relatively strong with a small variance/range.
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** The trend line seems to be relatively weaker with a larger variance/range, so it is hard to tell if there is a strong relationship between the percentage of votes for Trump and the PPE amount per employee. There is an outlier on the left-hand part of the graph, and all of the points seem to be varying all over the place.
```{r}
...
**Answer** There appears to be a stronger relationship between the percentage of votes for Trump and the percentage of businesses recieving PPE compared to the PPE amount per employee (0.522 > 0.221).
...
**Answer** It tries to take the data for pct_trump and businesses_receiving_ppe_pct and tries to find a correlation coefficient, which is about 0.522. The p-value calulated here is very low. 
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer** It tries to take the data for pct_trump and businesses_receiving_ppe_pct and tries to find a correlation coefficient, which is about 0.221. While the p-value looks pretty small, it seems very high for our standards. 
```{r}
...
**Answer** We get the averages for the PPP amount per employee and the percentage of businesses receiving PPE corresponding to Clinton votes and Trump votes.
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer** The t-value is negative and somewhat low, and the p-value calculated is pretty high, so it appears that the relationship is not very meaningful.
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** The t-value is negative and pretty strong, The p-value test is pretty low, so there does appear to be a relationship between the businesses receiving PPE and the percentages of 2016 votes for both candidates. It's possible that the candidates have better access to bank loans for their campaigns.
```{r}
