...
**Answer** I noticed that while we were able to extract all of the news posted thus far in 2023, the date column mentions the full dates written out rather than being in a proper format. Also, the column names are in proper capitalization with the second column having a slash in its name, so they should be cleaned.
...
**Answer** I would add a column denoting if an item is a "Media Advisory" or if it relates to any of the judges throughout Maryland, as well as adding a URL for each story and some of the text.
...
**Answer** Some of the items are part of the menu/web interface and we don't actually get to the stories until around item no. 17.
...
**Answer** The output mentions the text in the list items, including references to the website's menu interface and even the press releases with their date of reporting.
...
**Answer** There is now a dataset which collects all of the text in the list items into one column, but there is still some extraneous text alongside the press releases, and the text with the press releases still have their written date in them.
