...
**Answer** The dataset has four columns and 3,822 rows.
...
**Answer** The code is reading the entries in the "date" column, taking the month in the date and counting the number of calls that occurred in that month. I would assume "label=TRUE" lists the abbreviation for that month rather than using its number ("Jan" for January instead of "01," "Feb" for February instead of "02," etc.). Then, it shows the number of calls for each month and arranges the months in the order of highest to lowest number of calls.
...
**Answer** I see an empty semi-white box.
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer** I now see bars of varying heights along with a labeled x-axis ("month") and a labeled y-axis ("count"). The solid grey background changes to a pattern of squares/rectangles with lines.
...
**Answer** The bars have now been rearranged from shortest to tallest, with the corresponding months labeled below them. The "month" title on the x-axis has been renamed to "reorder(month, total_calls)."
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** The x-axis and y-axis have been swapped, and the bars extend from left to right now. The bars are now sorted from longest to shortest (from top-down).
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** The dark grey background behind the bars has been removed.
...
**Answer** The outer background is now light blue and white lines run through the middle of the bars from left to right. The x-axis and y-axis labels are slightly larger and in a darker font and there is now a darker line at the bottom where the x-axis is.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** The graph now has a title, both the x-axis and y-axis have been renamed to "month" and "total calls" respectively, and there is now a caption that mentions the source of the data.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** The data entries are sorted by the date rather than by the number of calls, which makes it harder to find out which date has the highest and lowest number of calls at first. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** There is now a line graph showing the number of drug overdose calls in Balimore County over a certain period of time. There is a larger congregation of calls between the 5 and 15 spots on the y-axis, meaning on average, there would be 5 to 15 drug overdose calls in Baltimore County during a given day.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** There are breaks for every week on the x-axis, but the text overlaps with one another to the point where it's practically unreadable.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** The labels on the x-axis are now at an angle, making them slightly more readable (although there is still some overlap).
